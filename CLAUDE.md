...
„Knowledge-Hub Gesundes Altern & Longevity" — ein Rechercheportal zum Themenfeld Gesundes Altern & Longevity. Ein Angebot von **Monitor Versorgungsforschung** (Betreiber: eRelation AG – Content in Health, Bonn).

Live: https://longevity.m-vf.de/
...
py vorlage-abgleich.py ..\longevity-portal --uebernehmen
```
...
| Live prüfen | `curl -s "https://longevity.m-vf.de/?cb=$(date +%s)"` — Cache-Buster nötig |
| Pages-Status | `gh api repos/mvf-portal/longevity-portal/pages/builds/latest` |
...
|---|---|---|
| `LONGEVITYHUB` | Claude-API für die Studienauswahl | Der tägliche Lauf bricht ab, `index.html` bleibt unverändert |
| `LONGEVITYHUBMC` | Mailchimp-API für den Kampagnen-Entwurf | Nur dieser Schritt entfällt (`continue-on-error`) |
...
**Alle Portale schreiben in dasselbe Mailchimp-Konto.** `mailchimp_entwurf.py` erkennt seine eigenen Kampagnen am Titel-Präfix `MVF Longevity-Newsletter`. Der Präfix muss sich **vollständig** von den Schwesterportalen unterscheiden: `datum_aus_titel()` prüft mit `startswith()`, ein bloßer Zusatz reicht nicht.

Mailchimp-Gruppe dieses Portals: `group[16140][16384]` („Studien Newsletter Longevity"). **Gruppen-Nummern sind Identitäten, keine Beschriftungen** — wer eine umbenennt, verschiebt Menschen, nicht Wörter.

Die Anmeldeseite `newsletter.html` kennt diese Nummer seit dem 01.09.2026 nicht mehr. Sie sendet an den Anmelde-Endpunkt auf dem MVF-Server (`/anmeldung/anmeldung.php`, beschrieben in `mvf-server/anmeldung/LIESMICH.md` im Repo `knowledge-hubs`) und nennt dort nur den Schlüssel dieses Hubs: `longevity`. Welche Gruppe dahintersteht, weiß allein der Server — **ein Schlüssel, der nicht in seiner Zulassungsliste steht, wird abgewiesen.** Ein neuer Hub braucht deshalb drei Eintragungen in dieser Reihenfolge: Gruppe in Mailchimp, Kennung in `anmeldung-zugang.php`, Schlüssel hier.
